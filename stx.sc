...
global_containers = [...filter(global_items, _~'shulker_box' != null), 'chest', 'double_chest', 'barrel', 'hopper', 'dropper', 'dispenser'];
...
global_air_mode = config:'air_mode' || 'default'; // default | dummy_item
...
__config() -> (
    config = {
        ...
        'commands' -> {
            ...
            'item_list create <name> layout <item_layout>' -> 'createItemListFromItemLayout',
            ...
            'item_list edit <item_list> remove items <item_list_entries>' -> 'removeEntriesFromItemListFromItems',
            ...
            //'item_list view <item_list>' -> 'viewItemList',

            'settings' -> 'settings',
            'settings shulker_box_color' -> 'printShulkerBoxColor',
            'settings shulker_box_color <shulker_box_color>' -> 'setShulkerBoxColor',
            'settings item_frame_type' -> 'printItemFrameType',
            'settings item_frame_type <item_frame_type>' -> 'setItemFrameType',
            'settings filter_item_amount' -> 'printFilterItemAmount',
            'settings filter_item_amount <item_amount>' -> 'setFilterItemAmount',
            'settings double_chest_direction' -> 'printDoubleChestDirection',
            'settings double_chest_direction <direction>' -> 'setDoubleChestDirection',
            'settings extra_containers_mode' -> 'printExtraContainersMode',
            'settings extra_containers_mode <extra_containers_mode>' -> 'setExtraContainersMode',
            'settings invalid_items_mode' -> 'printInvalidItemsMode',
            'settings invalid_items_mode <invalid_items_mode>' -> 'setInvalidItemsMode',
            'settings air_mode' -> 'printAirMode',
            'settings air_mode <air_mode>' -> 'setAirMode',
            'settings placing_offset' -> 'printPlacingOffset',
            'settings placing_offset <offset>' -> 'setPlacingOffset',
            'settings skip_empty_spots' -> 'printSkipEmptySpots',
            'settings skip_empty_spots <bool>' -> 'setSkipEmptySpots',
            'settings replace_blocks' -> 'printReplaceBlocks',
            'settings replace_blocks <bool>' -> 'setReplaceBlocks',
            'settings minimal_filling' -> 'printMinimalFilling',
            'settings minimal_filling <bool>' -> 'setMinimalFilling',
            ...
            //'item_filter view <item_filter>' -> 'viewItemFilter',
            //'item_filter view <item_filter> <item>' -> 'viewItemFilter',
            'item_filter <item_filter> fill <from_pos> <to_pos> items <stackable_items>' -> 'fillItemFiltersFromItems',
            'item_filter <item_filter> fill <from_pos> <to_pos> list <item_list>' -> 'fillItemFiltersFromItemList',
            'item_filter <item_filter> fill <from_pos> <to_pos> layout <item_layout>' -> 'fillItemFiltersFromItemLayout',
            'item_filter <item_filter> give items <stackable_items>' -> 'giveItemFilterFromItems',
            'item_filter <item_filter> give list <item_list>' -> 'giveItemFilterFromItemList',
            'item_filter <item_filter> give layout <item_layout>' -> 'giveItemFilterFromItemLayout',

            'container empty <from_pos> <to_pos>' -> 'fillContainersEmpty',
            'container empty <from_pos>' -> ['fillContainersEmpty', null],
            'container empty' -> ['fillContainersEmpty', null, null],

            'container empty_boxes <from_pos> <to_pos>' -> ['fillContainersEmptyBoxes', 1],
            'container empty_boxes <from_pos>' -> ['fillContainersEmptyBoxes', null, 1],
            'container empty_boxes' -> ['fillContainersEmptyBoxes', null, null, 1],
            'container empty_boxes_stacked <from_pos> <to_pos>' -> ['fillContainersEmptyBoxes', 64],
            'container empty_boxes_stacked <from_pos>' -> ['fillContainersEmptyBoxes', null, 64],
            'container empty_boxes_stacked' -> ['fillContainersEmptyBoxes', null, null, 64],

            'container full fill <from_pos> <to_pos> items <items>' -> ['fillContainersFromItems', 'full'],
            'container full fill <from_pos> <to_pos> list <item_list>' -> ['fillContainersFromItemList', 'full'],
            'container full fill <from_pos> <to_pos> layout <item_layout>' -> ['fillContainersFromItemLayout', 'full'],
            'container full give <container> items <items>' -> ['giveContainerFromItems', 'full'],
            'container full give <container> list <item_list>' -> ['giveContainerFromItemList', 'full'],
            'container full give <container> layout <item_layout>' -> ['giveContainerFromItemLayout', 'full'],

            'container full_boxes fill <from_pos> <to_pos> items <items>' -> ['fillContainersFromItems', 'full_boxes'],
            'container full_boxes fill <from_pos> <to_pos> list <item_list>' -> ['fillContainersFromItemList', 'full_boxes'],
            'container full_boxes fill <from_pos> <to_pos> layout <item_layout>' -> ['fillContainersFromItemLayout', 'full_boxes'],
            'container full_boxes give <container> items <items>' -> ['giveContainerFromItems', 'full_boxes'],
            'container full_boxes give <container> list <item_list>' -> ['giveContainerFromItemList', 'full_boxes'],
            'container full_boxes give <container> layout <item_layout>' -> ['giveContainerFromItemLayout', 'full_boxes'],

            'container prefill fill <from_pos> <to_pos> items <items>' -> ['fillContainersFromItems', 'prefill'],
            'container prefill fill <from_pos> <to_pos> list <item_list>' -> ['fillContainersFromItemList', 'prefill'],
            'container prefill fill <from_pos> <to_pos> layout <item_layout>' -> ['fillContainersFromItemLayout', 'prefill'],
            'container prefill give <container> items <items>' -> ['giveContainerFromItems', 'prefill'],
            'container prefill give <container> list <item_list>' -> ['giveContainerFromItemList', 'prefill'],
            'container prefill give <container> layout <item_layout>' -> ['giveContainerFromItemLayout', 'prefill'],

            'container prefill_unstackables fill <from_pos> <to_pos> items <items>' -> ['fillContainersFromItems', 'prefill_unstackables'],
            'container prefill_unstackables fill <from_pos> <to_pos> list <item_list>' -> ['fillContainersFromItemList', 'prefill_unstackables'],
            'container prefill_unstackables fill <from_pos> <to_pos> layout <item_layout>' -> ['fillContainersFromItemLayout', 'prefill_unstackables'],
            'container prefill_unstackables give <container> items <items>' -> ['giveContainerFromItems', 'prefill_unstackables'],
            'container prefill_unstackables give <container> list <item_list>' -> ['giveContainerFromItemList', 'prefill_unstackables'],
            'container prefill_unstackables give <container> layout <item_layout>' -> ['giveContainerFromItemLayout', 'prefill_unstackables'],
            ...
            'item_frame place <from_pos> <to_pos> <facing> list <item_list>' -> 'placeItemFramesFromItemList',
            'item_frame place <from_pos> <to_pos> <facing> layout <item_layout>' -> 'placeItemFramesFromItemLayout',
            'item_frame give items <items>' -> 'giveItemFramesFromItems',
            'item_frame give list <item_list>' -> 'giveItemFramesFromItemList',
            'item_frame give layout <item_layout>' -> 'giveItemFramesFromItemLayout',

            'chests full_boxes items <items>' -> ['giveChestsFromItems', 'full_boxes'],
            'chests full_boxes list <item_list>' -> ['giveChestsFromItemList', 'full_boxes'],
            'chests full_boxes layout <item_layout>' -> ['giveChestsFromItemLayout', 'full_boxes'],
            'chests stacks items <items>' -> ['giveChestsFromItems', 'stacks'],
            'chests stacks list <item_list>' -> ['giveChestsFromItemList', 'stacks'],
            'chests stacks layout <item_layout>' -> ['giveChestsFromItemLayout', 'stacks'],
            'chests single_items items <items>' -> ['giveChestsFromItems', 'single_items'],
            'chests single_items list <item_list>' -> ['giveChestsFromItemList', 'single_items'],
            'chests single_items layout <item_layout>' -> ['giveChestsFromItemLayout', 'single_items'],

            'full_box <item>' -> ['getFullBox', null],
            'full_box <item> <shulker_box_color>' -> 'getFullBox',

            'bundle items <items>' -> 'getBundleFromItems',
            'bundle list <item_list>' -> 'getBundleFromItemList',
            'bundle layout <item_layout>' -> 'getBundleFromItemLayout'
        },
        'arguments' -> {
            'name' -> {
                'type' -> 'term',
            ...
            'item_lists' -> {
                ...
                'suggester' -> _(args) -> (
                    ...
                    item_list_string = join(' ', entries);
                    return(if(entries, map(item_lists, str('%s %s', item_list_string, _)), item_lists));
                ),
                'case_sensitive' -> false
            },
            'item_list_entries' -> {
                ...
                'suggester' -> _(args) -> (
                    ...
                    items = map(_readItemList(item_list), _:0);
                    input = args:'items';
                    entries = split(' ', input);
                    if(entries && slice(input, -1) != ' ', delete(entries, -1));
                    entry_string = join(' ', entries);
                    return(if(entries, map(items, str('%s %s', entry_string, _)), items));
                ),
                'case_sensitive' -> false
            },
            'item_filter' -> {
            ...
            'items' -> {
                ...
                'suggester' -> _(args) -> (
                    ...
                    entries = split(' ', input);
                    if(entries && slice(input, -1) != ' ', delete(entries, -1));
                    items_string = join(' ', entries);
                    return(if(entries, map(global_items, str('%s %s', items_string, _)), global_items));
                ),
                'case_sensitive' -> false
            },
            'stackable_items' -> {
                ...
                'suggester' -> _(args) -> (
                    ...
                    if(entries && slice(input, -1) != ' ', delete(entries, -1));
                    items_string = join(' ', entries);
                    items = filter(global_items, stack_limit(_) != 1);
                    return(if(entries, map(items, str('%s %s', items_string, _)), items));
                ),
                'case_sensitive' -> false
            }
        },
    ...
    if(_checkVersion('1.4.57'),
        config:'commands':'item_list view <item_list>' = 'viewItemList';
        config:'commands':'item_filter <item_filter> view' = ['viewItemFilter', null];
        config:'commands':'item_filter <item_filter> view <item>' = 'viewItemFilter';
    );
    return(config);
);
...
_handleInvalidItems(items) -> (
    invalid_items = filter(map(items, _:0), global_items~_ == null);
    if(invalid_items,
        if(
...
_handleUnstackableItems(items) -> (
    unstackable_items = filter(map(items, _:0), global_items~_ != null && stack_limit(_) == 1);
...
setShulkerBoxColor(shulker_box_color) -> (
...
setItemFrameType(item_frame_type) -> (
...
setDoubleChestDirection(direction) -> (
...
setExtraContainersMode(extra_containers_mode) -> (
...
setInvalidItemsMode(invalid_items_mode) -> (
...
listModules() -> (
    ...
    texts = [
        ...
        ...reduce(global_modules, module = _:'name'; [..._a, 'f 【', ...if(app_list~module != null, ['#2ECC71 ✔', '^#2ECC71 Installed'], ['#3498DB ↓', '^#3498DB Click to install', str('?/%s modules install %s', system_info('app_name'), module)]), 'f 】 ', str('%s %s', _:'color', _:'display_name'), str('^%s %s.sc', _:'color', module), ' \n'], []),
        'fs ' + ' ' * 80
    ];
    print(format(texts));
);
...
installModule(module) -> (
    if(!first(global_modules, _:'name' == module), _error('There is no module with that name'));
    ...
    result = run(str('script download %s', module));
    run(str('carpet scriptsAppStore %s', current_appstore));

    if(result:2, exit(print(format('r There was an error while trying to download the module', str('^r %s', result:2)))));
    print(format('f » ', 'g Successfully installed the ', str('w %s', module)))
);
...
createItemList(name, items) -> (
    ...
    if(list_files('item_lists', 'shared_text')~item_list_path != null, _error('An item list with that name exists already'));
...
deleteItemList(item_list, confirmation) -> (
    ...
    if(!confirmation, exit(print(format('f » ', 'g Click ', str('%su here', global_color), str('^g /%s item_list delete %s confirm', system_info('app_name'), item_list), str('!/%s item_list delete %s confirm', system_info('app_name'), item_list),'g  to confirm the deletion of the ', str('%s %s', global_color, item_list), 'g  item list'))));
...
editItemList(item_list) -> (
    ...
    print(format(reduce(_readItemList(item_list), [item, nbt] = _; item_string = _itemToString(_); [..._a, ' \n  ', '#EB4D4Bb ❌', '^r Remove item', str('?/%s item_list edit %s remove items %s', system_info('app_name'), item_list, item_string), '  ', if(nbt, ...[str('g %s*', item), str('^g %s', nbt)], str('g %s', item)), str('!/give @s %s', item_string)], ['f » ', 'g Edit the ', str('%s %s', global_color, item_list), 'g  item list: ', '#26DE81b (+)', '^#26DE81 Add more items', str('?/%s item_list edit %s add items', system_info('app_name'), item_list)])));
);
...
removeEntriesFromItemList(item_list, entries) -> (
    ...
    if(length(items) == l, _error('No entries to remove from the item list'));

    delete_file(item_list_path, 'shared_text');
    write_file(item_list_path, 'shared_text', map(items, _itemToString(_)));

    print(format('f » ', 'g Successfully removed the items from the ', str('%s %s', global_color, item_list), 'g  item list'));
);
...
infoItemList(item_list) -> (
    ...
    items = _readItemList(item_list);
    texts = reduce(items, [item, nbt] = _; [..._a, if(length(items) < 12, ' \n', if(_i == 0, '', 'g , ')), str('g %s', if(length(items) < 12, '   ', '')), str('g%s %s%s', if(global_items~item == null, 's', ''), item, if(nbt, '*', '')), str('!/give @s %s', item + (nbt || '')), if(nbt, str('^g %s', nbt), '')], ['f » ', 'g The ', str('%s %s', global_color, item_list), 'g  item list contains the following items: ']);
    print(format(texts));
);
...
viewItemList(item_list) -> (
    ...
    items = filter(entries, _:0 != 'air' && global_items~(_:0) != null);
    ...
    screen = create_screen(player(), 'generic_9x6', item_list, _(screen, player, action, data, outer(pages), outer(items)) -> (
        ...
        if((action != 'clone' && data:'slot' < 45) || (45 <= data:'slot' <= 53), return('cancel'));
    ));
...
_ssiItemFilter(hopper, item_tuple, signal_strength, amount, minimal) -> (
    [item, nbt] = item_tuple;
    dummy_amount = floor((ceil(5 * 64 / 14 * (signal_strength - 1)) - (amount + 1) * (64 / stack_limit(item))) / (64 / stack_limit(global_stackable_dummy_item:0)));

    inventory_set(hopper, 0, if(minimal, 1, amount), item, nbt);
    inventory_set(hopper, 1, dummy_amount - 3, ...global_stackable_dummy_item);
    for(range(2, 5), inventory_set(hopper, _, 1, ...global_stackable_dummy_item));
...
fillItemFilters(item_filter, items, from_pos, to_pos) -> (
    ...
    _handleInvalidItems(items);
    _handleUnstackableItems(items);
    if(global_invalid_items_mode == 'skip', items = filter(items, global_items~(_:0) != null && stack_limit(_:0) != 1));
    ...
    affected_blocks = for(_scanStrip(from_pos, to_pos),
        ...
        if(global_items~item == null || stack_limit(item) == 1,
...
viewItemFilter(item_filter, item_tuple) -> (
    ...
    [item, count, nbt] = item_tuple || ['redstone', null, null];
    ...
    screen = create_screen(player(), 'hopper', str('%s Item Filter', global_item_filters:item_filter || item_filter), _(screen, player, action, data) -> (
        if(action != 'clone' && data:'slot' < 5, return('cancel'))
    ));
...
fillContainers(mode, items, from_pos, to_pos) -> (
    ...
    _handleInvalidItems(items);
    if(global_invalid_items_mode == 'skip', items = filter(items, global_items~(_:0) != null));
    ...
    print(format([
        'f » ',
        if(mode == 'full' || mode == 'full_boxes', 'g Completely filled ', mode == 'prefill' || mode == 'prefill_unstackables', 'g Prefilled '),
        str('%s %s', global_color, affected_blocks),
        str('g  container%s', if(affected_blocks == 1, '', 's')),
        if(mode == 'full_boxes', 'g  with full boxes', mode == 'prefill_unstackables', 'g  with unstackables', mode == 'full' || mode == 'prefill', '')
    ]));
);
...
fillMISChests(from_pos, to_pos, item_list_string) -> (
    ...
    affected_blocks = for(_scanStrip(from_pos, to_pos),
        ...
        // add warning if length of items > 54 ?
        if(global_invalid_items_mode == 'skip', items = filter(items, global_items~(_:0) != null && stack_limit(_:0) != 1));
...
giveMISChest(item_list_string) -> (
    ...
    invalid_items = filter(map(all_items, _:0), global_items~_ == null);
    if(invalid_items, _error(str('Invalid items: %s', join(', ', sort(_removeDuplicates(invalid_items))))));
...
cupholder() -> (
    heads = ['eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvYzJhM2VkZWJiMGM4ZGNmODdmNWUyOTU4YTYzMjBlNjdiM2FjYjk1Yzg0YjVhZjUzZDY1Zjc2ZWNhMThkMjFmYiJ9fX0=', 'eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvZmU4ZDE5NGEwNDYxYWVjMTVlZDJiNmE1YzhjMDIwNzliNWJjN2M2ZGFmYmU2MGM1NjVkZDEyYjhlMGQ3YTZjYiJ9fX0=', 'eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvZTAyN2Q0YTg2NDVlYzc4YWZhNjIzZmU0MjkwN2YyZGI2NjQxYmZlZjFiNDk5ZmU3N2IzNjBhMWQwYjlhNjMzYyJ9fX0=', 'eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvNmJhOTQxYTgxYjc3NTliMDZlOThhYmZkMDkwOGU4Y2I1ZDJlM2U3MjBhYzBhOGY0ZGFiMjM5N2VmNzQ5NTgyOSJ9fX0=', 'eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvZDFkNjk2MDgwNmZhZjc0M2NmYjkwNzYzODQwZjAxZWRkOTg5YzZhNDFiZDQzZGJjYzUxZjNiMWIwOTQ4YTUyZCJ9fX0=', 'eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvMjQyODc1ZjQwYjAyYTUyMzQyNjhjNjg0ZWRiOWQ4ZGRjN2VkYTIzOTI5ODRhZGFiZWFhZmEyMzU0NzA4ZGNmNiJ9fX0=', 'eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvMmUwODQ0M2NmNjNlYjlmNmQyZmM2NDhlZWU5OTA0NGIyYTAzYTcyMzM5MGRmZGU2MWM2MWM4MjgyZjA0NjM5MiJ9fX0='];
    screen = create_screen(player(), 'generic_9x6', 'Cupholder', _(screen, player, action, data) -> (
        if(action != 'clone' && data:'slot' < 54, return('cancel'));
    ));
    loop(54, inventory_set(screen, _, 1, 'player_head', str('{display:{Name:\'{"text":"Cup #%d","italic":false}\'},SkullOwner:{Id:[I;0,0,0,0],Properties:{textures:[{Value:"%s"}]}}}', _ + 1, rand(heads))));
);
...
__on_player_right_clicks_block(player, item_tuple, hand, block, face, hitvec) -> (
    ...
    [item, count, nbt] = item_tuple;
    if((item == 'item_frame' || item == 'glow_item_frame') && has(nbt, 'stx.item_frames'),
        origin_pos = pos_offset(block, face);
        player_facing = _getFacing(player);
        data = nbt:'stx':'item_frames';
        __on_item_frame_placed(item, origin_pos, face, player_facing, data);
    );
);
